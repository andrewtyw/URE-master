...
def tacred2mnli_neutral(
    ...
    for neg_template in sorted_template: #遍历所有的template, 找出可以用来finetune的template
        if neg_template=='{subj} and {obj} are not related':
            continue
        neg_template_label = template2label[neg_template]   #  这里会出现.号的问题.......
        #assert len(neg_template_label)!=0  # 必须要能在template2label找到这个template对应的label
...
def get_tac_pos_neg_templates(config_path:str):
    ...
    if config_path.find("wiki")!=-1: #wiki
        ...
        # and remove duplicate templates(cause one may be shared by multiple relatoins) 
        templates = [
